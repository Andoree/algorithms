...
def main():
    ...
    optimal_path.append((initial_node, -1))
    for i in range(n - 1, -1, -1):
        optimal_path.append((initial_node, initial_node))
        if change_path_flags[initial_node][i] == True:
            previous_node = (initial_node + 1) % 2
            optimal_path.append((previous_node, initial_node))
            initial_node = previous_node
    optimal_path.append((-1, initial_node))
    ...
    for (from_node, to_node) in optimal_path:
        if to_node == -1:
            ...
            print(f"Change path: {from_node} -> {to_node}")
        else:
            print(f"Go forward: {from_node} -> {to_node}")
